# Importing and transforming data {#transform}
...
The example data can be downloaded from [here](link) as saved to our project
...
# Tell read_csv that missing values are the string NaN
# Clean up the variable names with clean_names() from the janitor package
dat <- readr::read_csv("070718-proteomics-example-data.csv",
                na = "NaN") %>%
                janitor::clean_names()
```
...
The first thing to do with any data set is to actually look at it!
